...
cobatch_list<-list()
sample_name_list<-c("A3","A4","N1","N2","N4")
for (sample_name in sample_name_list){
  # sample_name<-"A3"   ##test line 
...
range(placenta.Cobatch$nCount_Cobatch)#  89 10757
...
placenta.rna<-subset(x = target_final,subset = sample== sample_name)
...
p2 <- DimPlot(placenta.rna, group.by = "final_major_subgroup_brief", reduction = "tsne", label = TRUE, repel = TRUE) + NoLegend() +   ggtitle("scRNA-seq")
pcom1<-CombinePlots(plots = list(p1, p2))
p3 <- DimPlot(placenta.Cobatch, reduction = "umap") + NoLegend() + ggtitle("Cobatch")
p4 <- DimPlot(placenta.rna, group.by = "final_major_subgroup_brief", reduction = "umap", label = TRUE, repel = TRUE) + NoLegend() +   ggtitle("scRNA-seq")
...
cobatch_list<-c(cobatch_list,list(placenta.Cobatch))
...
DimPlot(coembed, reduction="umap", split.by = "tech", group.by = "final_major_subgroup_brief", label = TRUE, repel = TRUE) + NoLegend()
...
names(cobatch_list)<-sample_name_list
saveRDS(cobatch_list, file = "/mnt/data/chenwei/gongchen/chip_seq_data/gongcheng_data/intergrated_result/all_cobatch_list_sample_list.rds")
cobatch_meta<-rbind(cobatch_list[[1]]@meta.data[,c(1:7)],cobatch_list[[2]]@meta.data[,c(1:7)],cobatch_list[[3]]@meta.data[,c(1:7)],
      cobatch_list[[4]]@meta.data[,c(1:7)],cobatch_list[[5]]@meta.data[,c(1:7)])
write.csv(as.data.frame(cobatch_meta), file = "/mnt/data/chenwei/gongchen/chip_seq_data/gongcheng_data/intergrated_result/all_cobatch_list_sample_predicted_metadata.csv",quote=F, row.names=T) 

#Specially for A5 sample
sample_name<-"A5"   ##test line 
print(as.character(sample_name))
# 读取peak
#peaks <- Read10X_h5("/mnt/data/chenwei/gongchen/chip_seq_data/gongcheng_data/atac_v1_placenta_10k_filtered_peak_bc_matrix.h5")
#str(peaks)
...
#过滤掉cobatch数据中总count数低于500,大于12000的细胞？，这个阈值需要根据具体实验设置
range(placenta.Cobatch$nCount_Cobatch)#  232 19822
placenta.Cobatch <- subset(placenta.Cobatch, subset = nCount_Cobatch > 500 & nCount_Cobatch < 12000)
placenta.Cobatch2<-placenta.Cobatch
placenta.Cobatch$tech <- "Cobatch"
...
#placenta.rna <- readRDS("/mnt/data/chenwei/gongchen/3.seurat_result/intergrated_0713/target_final_A4_forced_cell_low_cell_double_both_remove_seurat.rds")
#target_final_N4<-subset(x = target_final,subset = sample== "N4")
placenta.rna<-subset(x = target_final,subset = sample== "A4")
#placenta.rna <- readRDS("/mnt/data/chenwei/gongchen/3.seurat_result/intergrated_0713/target_final_N4_forced_cell_low_cell_double_both_remove_seurat.rds")
DefaultAssay(placenta.rna) <- "RNA"
# Perform standard analysis of each modality independently RNA analysis
placenta.rna <- NormalizeData(placenta.rna)
...
write.csv(as.data.frame(placenta.Cobatch@meta.data)[,1:7], file = "/mnt/data/chenwei/gongchen/chip_seq_data/gongcheng_data/intergrated_result/A5_cobatch_list_sample_predicted_metadata.csv",quote=F, row.names=T) 
