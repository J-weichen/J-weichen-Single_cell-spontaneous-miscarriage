...
range(statdf$number)#  3 189
CTRL_heatmap<-ggplot(statdf,aes(x=indexa,y=indexb,fill=number))+geom_tile(color="white")+
  geom_text(aes(label = round(number, 1))) +
  scale_fill_gradientn(colours = c("#4393C3","#ffdbba","#B2182B"),limits=c(0,200))+
  scale_x_discrete("cluster 1 produces molecule 1")+
  scale_y_discrete("cluster 2 produces molecule 2")+
  theme_minimal()+labs(title = "CTRL interaction number")+
  theme(axis.text.x.bottom = element_text(hjust = 1, vjust = NULL, angle = 45),
        panel.grid = element_blank())
CTRL_heatmap
ggsave(CTRL_heatmap,filename = "/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/subcelltype/CTRL_interaction_number_no_merge.pdf",width = 15,height = 14)
...
CTRL_heatmap2<-ggplot(statdf,aes(x=indexa,y=indexb,fill=total_number))+geom_tile(color="white")+
  geom_text(aes(label = round(total_number, 1))) +
  scale_fill_gradientn(colours = c("#4393C3","#ffdbba","#B2182B"),limits=c(0,300))+
...
CTRL_heatmap2
ggsave(CTRL_heatmap2,filename = "/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/subcelltype/CTRL_interaction_number_merge.pdf",width = 15,height = 14)
...
head(statdf_dc_tri)
range(statdf_dc_tri$value)
CTRL_heatmap3<-ggplot(statdf_dc_tri, aes(indexa, variable)) + 
  geom_tile(aes(fill = value),colour = "white") +
  geom_text(aes(label = round(value, 1))) +
  scale_fill_gradientn(colours = c("#4393C3","#ffdbba","#B2182B"),limits=c(0,300))+
  scale_x_discrete("cluster 1")+scale_y_discrete("cluster 2")+
  theme_bw()+labs(title = "CTRL interaction number")+
  theme(
    axis.text.x.bottom  = element_text(hjust = 1, vjust = NULL, angle = 45),
    panel.grid = element_blank()
  )
CTRL_heatmap3
ggsave(CTRL_heatmap3,filename = "/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/subcelltype/CTRL_interaction_number_merge_up_tri.pdf",width = 15,height = 14)
...
color2=colorRampPalette(brewer.pal(9, "Purples")[3:7])(10)#将颜色分成多少份，取决于互作关系数目的最大值
...
pdf("/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/subcelltype/CTRL_subcell_net_merge_plot.pdf",width=12,height=12)
plot(net,
...
pdf("/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/subcelltype/CTRL_subcell_not_merge_network_R_plot.pdf",width=12,height=12)
...
#贝壳图
pdf("/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/subcelltype/CTRL_subcell_net_nomerge_plot_sperated.pdf",width=30,height=30)
length(unique(mynet2$SOURCE))#7
