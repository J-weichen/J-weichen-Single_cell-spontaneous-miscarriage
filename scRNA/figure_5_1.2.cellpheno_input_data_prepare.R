...
for ( group_name in mydata_list){
  ...
  meta_data <- sub_cell@meta.data[,c("BARCODE_new","re_annotation_TFac")]
  ...
  write.table(cellphonedb_count, paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_4/subcelltype/",group_name,"/cellphonedb_count.txt"), sep='\t', quote=F)
  write.table(meta_data, paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_4/subcelltype/",group_name,"/cellphonedb_meta.txt"), sep='\t', quote=F, row.names=F)
}
...
meta_data <- target_final@meta.data[,c("BARCODE_new","re_annotation_TFac")]
meta_data$BARCODE_new<-rownames(meta_data)
meta_data <- as.matrix(meta_data)
meta_data[is.na(meta_data)] = "Unkown" #  细胞类型中不能有NA
#head(meta_data)
colnames(meta_data)<-c("Cell","cell_type")
write.table(cellphonedb_count, "/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_4/subcelltype/all_cell/cellphonedb_count.txt", sep='\t', quote=F)
write.table(meta_data,"/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_4/subcelltype/all_cell/cellphonedb_meta.txt", sep='\t', quote=F, row.names=F)
