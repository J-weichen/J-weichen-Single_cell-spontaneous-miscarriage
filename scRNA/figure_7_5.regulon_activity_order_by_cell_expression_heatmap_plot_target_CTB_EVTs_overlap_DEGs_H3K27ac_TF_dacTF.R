...
#CTBs_CTRL  CTBs_USM EVTs_CTRL  EVTs_USM 
##for CTBs
##cell names get order by expression level
...
head(Exprs_CTBs2)
CTBs_cell_order<-rownames(Exprs_CTBs2)
...
Activity_CTBs2<-Activity_CTBs2[CTBs_cell_order,]
...
Average_activity_data<-Activity_CTBs3
range(Average_activity_data)##0.000000 2.947309
#Average_activity_data[Average_activity_data == "Inf"]<- 2000
p1<-pheatmap(Average_activity_data, cluster_row =FALSE,cluster_col =FALSE,na_col = "gray",
...
p2<-pheatmap(Average_activity_data, cluster_row =FALSE,cluster_col =FALSE,na_col = "gray",
             clustering_distance_rows ="euclidean",#correlation
             show_rownames = T,show_colnames = F,
             annotation_col = annotation_col,#annotation_row=annotation_row,
             annotation_colors = ann_colors, 
...
pdf("/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/regulon_order_by_cellexpression_noscaled_CTBs_TEF_TFAP2A_heatmap_plot.pdf",width = 7,height=3)
print(p1)
dev.off()
pdf("/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/regulon_order_by_cellexpression_scaled_CTBs_TEF_TFAP2A_heatmap_plot.pdf",width = 7,height=3)
print(p2)
dev.off()
###scale by row
head(Activity_CTBs2)
...
htkm <- Heatmap(plot_matrix,name= "z-score",border = TRUE,top_annotation = column_ha, 
                ...
                cluster_column_slices = FALSE,column_title = "smooth regulon activity in CTBs(order by expression)", 
...
print(htkm)
pdf("/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/regulon_order_by_gene_expression_smooth_CTBs_TEF_TFAP2A_heatmap_plot.pdf",width = 7,height=3)
print(htkm)
dev.off()
...
head(Exprs_EVTs2)
EVTs_cell_order<-rownames(Exprs_EVTs2)

##for regulon activity
...
Activity_EVTs2<-Activity_EVTs2[EVTs_cell_order,]
...
Average_activity_data<-Activity_EVTs3
range(Average_activity_data)##0.00000 3.82654
#Average_activity_data[Average_activity_data == "Inf"]<- 2000
p1<-pheatmap(Average_activity_data, cluster_row =FALSE,cluster_col =FALSE,na_col = "gray",
             clustering_distance_rows ="euclidean",#correlation
             show_rownames = T,show_colnames = F,
...
pdf("/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/regulon_order_by_gene_expression_noscaled_EVTs_ATF3_TEAD1_GATA2_SNAI1_TFAP2A_HSF1_ELF3_SMAD3_ARNT2_heatmap_plot.pdf",width = 6,height=6)
print(p1)
dev.off()
pdf("/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/regulon_order_by_gene_expression_scaled_EVTs_ATF3_TEAD1_GATA2_SNAI1_TFAP2A_HSF1_ELF3_SMAD3_ARNT2_heatmap_plot.pdf",width = 6,height=6)
print(p2)
dev.off()
...
htkm <- Heatmap(plot_matrix,name= "z-score",border = TRUE,top_annotation = column_ha, 
                ...
                row_title_rot= 0,cluster_rows= FALSE,cluster_row_slices= FALSE,cluster_columns= FALSE)
print(htkm)
pdf("/mnt/data/chenwei/gongchen/manuscript/manu_figure/major_4/regulon_order_by_gene_expression_smooth_EVTs_ATF3_TEAD1_GATA2_SNAI1_TFAP2A_HSF1_ELF3_SMAD3_ARNT2_heatmap_plot.pdf",width = 7,height=3)
print(htkm)
dev.off()
