...
#major_maker<-c("VIM","HLA-B","KRT7","PERP","AIF1","LYVE1","DCN","DLK1","PECAM1","EPCAM","PAEP","NKG7","CD3D","CD8A","IL7R","CD79A","HBA1","MS4A2")
major_maker<-c("VIM","KRT7","AIF1","DCN","PECAM1","PAEP","NKG7","CD3D","CD79A","HBA1","MS4A2")
immune_maker<-c("VIM","CDH1","EGFR","HLA-G","MMP11","CGA","CYP19A1","CSH2","ERVFRD-1","DCN","DLK1","THY1","COL1A1","LAMA2","TIMP1",
...
plot_umap_maker <- FeaturePlot(object = target_final, features = major_maker,cols= c("grey", "red"),ncol=6)
ggsave("/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/Umap_final_major_maker.png", plot_umap_maker,width=30, height=15)

plot_Ridge_maker <-RidgePlot(target_final,group.by = "final_major_group_brief", features = major_maker, ncol = 6)
ggsave("/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/Ridge_final_major_maker.png", plot_Ridge_maker,width=30, height=15)

#maker plot
#小提琴图
#ref:https://www.rdocumentation.org/packages/Seurat/versions/3.1.0/topics/FetchData
#CR maker
maker_Vln_plots <- VlnPlot(target_final, features = rev(major_maker[-c(1:2)]),group.by = "final_major_group_brief",cols = ppCor_all2,pt.size = 0, combine = FALSE)
maker_Vln_plots2<-CombinePlots(plots = maker_Vln_plots, ncol =1)
ggsave("/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/Violin_final_major_maker.png", maker_Vln_plots2,width=20, height=20)

exprs <- data.frame(FetchData(object = target_final,slot = "data",vars = c(major_maker[-c(1:2)],"Treat","final_major_group_brief")))
data_plot <- melt(exprs)
data_plot$final_major_group_brief<- factor(data_plot$final_major_group_brief,levels = rev(major_order))
maker_plot_line<-ggplot(data_plot,aes(x = final_major_group_brief,y=value,fill = final_major_group_brief)) + 
...
maker_plot_up<-ggplot(data_plot,aes(x = final_major_group_brief,y=value,fill = final_major_group_brief)) + 
  geom_violin(scale = "width") + #geom_jitter(size = 0.00)+
...
maker_plot_up
maker_plot_line
ggsave("/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/Violin_final_major_maker1.png", maker_plot_up,width=12, height=18)
ggsave("/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/Violin_final_major_maker2.png", maker_plot_line,width=20, height=15)
...
#Aver_plot<-top10
#2.经典气泡图
Idents(object = target_final) <- "final_major_group_brief"
DotPlot(target_final, features = major_maker,group.by ="final_major_group_brief",cols = c("blue","red")) + RotatedAxis()
DotPlot(target_final, features = major_maker,group.by ="final_major_group_brief",split.by = "Treat",cols =  c("blue","red"),assay = "RNA") + RotatedAxis()
DotPlot(target_final, features = major_maker,split.by = "Treat",cols = ppCor,assay = "RNA") + RotatedAxis()

##3. heatmap for averageExpression
Aver_plot<-major_maker
AverageExp<-AverageExpression(target_final,assays="RNA",slot = "data",features=major_maker)
typeof(AverageExp);
head(AverageExp$RNA)
...
AverageExp_data<-AverageExp_data[,major_order]
...
scaled_data <- target_final@assays$SCT@scale.data;dim(scaled_data)# 2981 53232
makergenes<-major_maker
...
target_1K$final_major_group_brief <- factor(x =target_1K$final_major_group_brief, levels = major_order)
...
PH<-DoHeatmap(object = target_1K, features =gene_panel_intersect,draw.lines = TRUE,label = F,
              ...
              group.by ="final_major_group_brief",group.bar.height = 0.1,group.colors=ppCor[1:length(levels(target_1K$final_major_group_brief))]) 
PH2<-PH+scale_fill_gradientn(colors = c("blue", "white", "red"))+theme(axis.text.y = element_text(size = 10))+NoLegend()
ggsave(PH2,file="/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/major_maker_heatmap.png",width = 12,height = 10)
ggsave(PH,file="/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/major_maker_heatmap0.png",width = 12,height = 10)
...
PH<-DoHeatmap(object = target_1K, features =major_maker,draw.lines = TRUE,label = F,
...
PH2
ggsave(PH2,file="/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/major_maker_heatmap1.png",width = 12,height = 10)
ggsave(PH,file="/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/major_maker_heatmap2.png",width = 12,height = 10)
...
gene_features <- major_maker
cluster_info <- target_final$final_major_group_brief
...
pdf("/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/major_maker_heatmap1.pdf")
draw(plot_HP, row_title = "Genes", 
     #     padding = unit(c(2, 0, 2, 0), "mm"),
     adjust_annotation_extension = TRUE,  # default
     column_title = paste0("major_maker_heatmap_SCT_data"))
dev.off()
...
colanno=target_final@meta.data[,c("sample_code","final_major_group_brief")]
colanno$final_major_group_brief=factor(colanno$final_major_group_brief,levels = major_order)
colanno$sample_code<-as.character(colanno$sample_code)
colanno=colanno%>%arrange(final_major_group_brief)
#rownames(colanno)=colanno$CB;colanno$CB=NULL
#先对细胞进行排序，按照celltype的顺序，然后对基因排序
#rowanno=markerdf1
#
#提取scale矩阵的行列时，按照上面的顺序
mark_gene<-major_maker
...
rowanno$major_cluster=factor(rowanno$major_cluster,levels = c("no_Trophoblast",major_order))
rowanno=rowanno%>%arrange(major_cluster)

mat2 <- mat1[as.character(rowanno$gene),]
#手动scale
mat3<-scale(t(mat2), center=T,scale=T)
mat4<-t(mat3)
range(mat4)
...
mat4[1:4,1:4]
pdf("/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/major_maker_heatmap2.pdf")

pheatmap(mat4[,rownames(colanno)],cluster_rows = F,cluster_cols = F,
         show_colnames = F,annotation_col = colanno,
         gaps_row=as.numeric(cumsum(table(rowanno$major_cluster))[-11]),
         gaps_col=as.numeric(cumsum(table(colanno$final_major_group_brief))[-10]))
...
Idents(object = target_final) <- 'final_major_group_brief'
makergenes<-major_maker
hm1 <- DoHeatmap(target_final,features =makergenes, cells = cells.1, draw.lines = TRUE,label = F,group.colors=ppCor_all[1:length(levels(target$major_cluster))])
hm2 <- DoHeatmap(target_final,features = makergenes, cells = cells.2, draw.lines = TRUE,label = F,group.colors=ppCor_all[1:length(levels(target$major_cluster))])
...
ggsave(hm22,file="/mnt/data/chenwei/gongchen/3.seurat_result/1.global_cell_maker/major_maker_treat_split_heatmap.png",width = 22,height = 10)
