...
cell.info_target_final<-ALL_coldata
...
compare_group<-"re_annotation_TFac"
cell_types_name <- names(table(cell.info_target_final[compare_group]))
cell_types_name1<- cell_types_name[which(!(cell_types_name %in% c("Masts","Ery")))]
...
for ( name_Cell in cell_types_name1){
  ...
  cells.CTRL <- cell.info_target_final %>% filter(re_annotation_TFac  == cell_identity) %>% filter(Treat  == "CTRL") %>%  rownames()
  cells.Abortion <- cell.info_target_final %>% filter(re_annotation_TFac  == cell_identity) %>% filter(Treat  == "Abortion") %>%  rownames()
  ...
  head(res.label_all)
  write.table(as.data.frame(res.label_all), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/",cell_identity,"_USM_def_TFs_p005.txt"),quote = F, row.names = T,sep = "\t")
  ...
  write.table(as.data.frame(res.label_all[which(res.label_all$p.adj<0.05),]), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/",cell_identity,"_USM_def_TFs_p005_q005.txt"),quote = F, row.names = T,sep = "\t")
  ...
  write.table(as.data.frame(TF_active_high), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/",cell_identity,"_USM_def_TFs_p005_active_mean_high.txt"),quote = F, row.names = T,sep = "\t")
...
saveRDS(D_ac_TFs_res_list, paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/",cell_name,"_D_ac_TFs_res_list.rds"))
saveRDS(D_ac_TFs_lable_list, paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/",cell_name,"_D_ac_TFs_lable_list.rds"))
saveRDS(D_ac_TFs_active_high_list, paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/",cell_name,"_D_ac_TFs_active_high_list.rds"))

D_ac_TFs_res_list<-readRDS(paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/",cell_name,"_D_ac_TFs_res_list.rds"))
D_ac_TFs_lable_list<-readRDS(paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/",cell_name,"_D_ac_TFs_lable_list.rds"))
D_ac_TFs_active_high_list<-readRDS(paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/",cell_name,"_D_ac_TFs_active_high_list.rds"))
...
head(D_ac_TF_matrix);tail(D_ac_TF_matrix);dim(D_ac_TF_matrix)#  5424    8
write.table(D_ac_TF_matrix, paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/merge_",cell_name,"_D_ac_TF_matrix.txt"), sep = "\t")
...
head(D_ac_TFs_active_high_matrix);tail(D_ac_TFs_active_high_matrix);dim(D_ac_TFs_active_high_matrix)#1026   13
write.table(D_ac_TFs_active_high_matrix, paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/Different_TFs/merge_",cell_name,"_D_ac_TFs_active_high_matrix.txt"), sep = "\t")
...
TF_Cell_net<-D_ac_TFs_active_high_matrix[,c("lable","cell_identity","change","foldchange")]
colnames(TF_Cell_net)<-c("daTFs_identity","cell_identity","change","foldchange")
...
write.table(as.data.frame(TF_Cell_net), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/TFs_cytoscape_file/",cell_name,"_TF_Cell_Network.txt"),quote = F, row.names = F,sep = "\t")
write.table(as.data.frame(Cell_node), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/TFs_cytoscape_file/",cell_name,"_Cell_node_stat.txt"),quote = F, row.names = F,sep = "\t")
write.table(as.data.frame(daTFs_node), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/TFs_cytoscape_file/",cell_name,"_daTFs_node_stat.txt"),quote = F, row.names = F,sep = "\t")
...
head(TF_Cell_net2);dim(TF_Cell_net2)# 1026   12
...
#write.table(as.data.frame(daTFs_node3), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/TFs_cytoscape_file/",cell_name,"_daTFs_node_stat2.txt"),quote = F, row.names = F,sep = "\t")
##then detail result for Cell-TFs_network showed in cytoscape
#daTFs_node3<-read.table(paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/TFs_cytoscape_file/",cell_name,"_daTFs_node_stat2.txt"),header = T)
...
#write.csv(as.data.frame(daTFs_node4), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/TFs_cytoscape_file/",cell_name,"_daTFs_Cell_relationship.csv"),quote = F, row.names = F,sep = ",")

################################################

##以下绘制样本中dacTFs变化的脚本本次不跑
#next step show relationship for def_acTFs and Cell subtypes
##evalute the intercell type relationship for changes in TFs
dim(TF_Cell_net)#  1064    4
...
clusters <- names(table(cell.info_target_final$re_annotation_TFac))

moduleScoreInCellType <- lapply(clusters, function(x) {
  cells.use <- rownames(subset(cell.info_target_final, re_annotation_TFac == x))
...
cell_target_final<-unique(as.character(target_final$re_annotation_TFac))
...
for (modules in module_name){
  ...
  ctMat_select$num<-ifelse(ctMat_select$re_annotation_TFac %in% Cell_select,1,0)
...
#target_genes             TFs 
#           8028                 19 
write.table(as.data.frame(TF_gene_final1[,1:3]), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/TFs_cytoscape_file/",cell_name,"_DEGs_overlap_TFs_Network.txt"),quote = F, row.names = F,sep = "\t")
write.table(as.data.frame(TF_gene_final_stat), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/TFs_cytoscape_file/",cell_name,"_DEGs_overlap_TFs_stat.txt"),quote = F, row.names = F,sep = "\t")
write.table(as.data.frame(special_daTFs_node0[,c(1,6,7)]), paste0("/mnt/data/chenwei/gongchen/manuscript/manu_table/major_figure_2/TFs_cytoscape_file/",cell_name,"_DEGs_overlap_TFs_stat2.txt"),quote = F, row.names = F,sep = "\t")
