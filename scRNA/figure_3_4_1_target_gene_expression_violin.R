...
Up_gene<-c("APOE","APOC1","CSH2","CTSD","SPP1","TFPI2","CSH1","FCER1G","HLA-A","HLA-DRA")
Down_gene<-c("HBZ","HBE1","HBG2","GAPDH","HBA1","XAGE3","COL1A1","HBA2","LY6E","PKM")   
gene_signature<-c(Up_gene,Down_gene,"SERPINE1")

gene_names<-"SERPINE1";y_value<-5.5
plot1<-VlnPlot(target_final, features = gene_names, y.max =y_value,  group.by = "re_annotation_TFac",pt.size = 0, split.by = "Treat",cols = ppCor[c(10,6)], combine =F) 
plot1_1<-plot1[[1]]+ stat_compare_means(method = "wilcox.test",data=plot1$data,label.y =y_value,label = "p.signif",hide.ns = F, label.x = 2)+geom_boxplot(width=0.15,outlier.shape = NA,notch=T,position=position_dodge(0.9),color="grey", alpha=0.4) +stat_summary(fun=mean, geom="point", shape=18, size=2, col="orange", position = position_dodge(0.9))
plot1_1
plot0<-FeaturePlot(object = target_final, features =  gene_names ,cols= c("grey", "red"),split.by = "Treat",pt.size = 1) 
plot0
plot<-FeaturePlot(object = target_final, features =  gene_names ,cols= c("grey", "red"),split.by = "sample",pt.size = 1,combine = F) 
...
target_gene2<-gene_signature[which(gene_signature %in% rownames(target_final))]
...
#for gene expression
express_merge_data3<-merge_data2[order(merge_data2[,target_gene],decreasing = F),]
express_merge_target<- express_merge_data3[,c("UMAP_1.x","UMAP_2.x","sample_code",target_gene)]
colnames(express_merge_target)<-c("UMAP_1","UMAP_2","sample_code","Expression")
head(express_merge_target)
pos_cell_umap<-ggplot(data=express_merge_target,mapping=aes(x=UMAP_1,y=UMAP_2,color = Expression))+
  geom_point(stat= "identity",size=1,alpha=0.5,show.legend = TRUE)+
  scale_color_gradient(low="darkgrey",high = "red")+
  labs(title =paste0("Expression of ",target_gene))+theme_bw()+
  theme(panel.border = element_rect(colour="grey",fill=NA),
        panel.grid=element_blank(),plot.title=element_text(hjust=0.5),
        axis.title = element_text(size=15),axis.text.x = element_text(size=10), 
        axis.text.y = element_text(size=15),legend.position = "right")#+facet_grid(.~sample_code)

pos_cell_umap+facet_wrap(~sample_code,ncol=3)
...
VlnPlot(target_final, features =gene_signature[3], pt.size = 0, split.by = "Treat",cols = ppCor[c(10,6)],ncol = 1)
...
plot1<-VlnPlot(target_final, features = gene_signature[1], y.max =2,  group.by = "re_annotation_TFac",pt.size = 0, split.by = "Treat",cols = ppCor[c(10,6)], combine =F) 
...
plot1<-VlnPlot(target_final, features = gene_signature[2], y.max =4,  group.by = "re_annotation_TFac",pt.size = 0, split.by = "Treat",cols = ppCor[c(10,6)], combine =F) 
...
genename <-"CD320"
print(genename)
#  VlnPlot(target_final, features =genename, pt.size = 0, split.by = "Treat",cols = ppCor[c(10,6)],ncol = 1)
merge_data_target<- data_plot[which(data_plot$variable==genename),]
