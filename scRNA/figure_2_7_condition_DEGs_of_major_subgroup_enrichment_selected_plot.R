...
length(unique(Enrich_all_plot2[which(Enrich_all_plot2$p.adjust<0.1),]$Description))#4033
length(unique(Enrich_all_plot2[which(Enrich_all_plot2$pvalue<0.05),]$Description))#4926

##selected terms pvalue<0.05
Enrich_all_plot2<-Enrich_all_plot2[which(Enrich_all_plot2$pvalue<0.05 & Enrich_all_plot2$p.adjust<0.1),]
dim(Enrich_all_plot2)# 18480    11
Enrich_all_plot2$LogPadjust<- c(-log10(Enrich_all_plot2$p.adjust))
range(Enrich_all_plot2$LogPadjust)# 0.5170067 54.8011257
...
dim(Enrich_all_number2);dim(Enrich_all_number)#2786   4033  
...
tail(Enrich_all_plot3);dim(Enrich_all_plot3)#18480    12
write.table(Enrich_all_plot3, file="/mnt/data/chenwei/gongchen/3.seurat_result/figure_two_result/enrichment_result/major_subgroup_all_GO_BP_merge_padjust01.txt",row.names=T, col.names=T) 
Enrich_all_plot3<-merge(Enrich_all_plot2,Enrich_all_number2)
tail(Enrich_all_plot3);dim(Enrich_all_plot3)#17233    12
write.table(Enrich_all_plot3, file="/mnt/data/chenwei/gongchen/3.seurat_result/figure_two_result/enrichment_result/major_subgroup_common_GO_BP_merge_padjust01.txt",row.names=T, col.names=T) 

#for no selected GO terms of change gene
Enrich_all_plot_noselect<-read.table("/mnt/data/chenwei/gongchen/3.seurat_result/figure_two_result/enrichment_result/major_subgroup_all_GO_BP_merge_padjust01.txt",header =T) 
head(Enrich_all_plot_noselect)
length(unique(Enrich_all_plot_noselect$Description))#4033
length(unique(Enrich_all_plot_noselect[which(Enrich_all_plot_noselect$Count>2),]$Description))#3148 
...
head(Enrich_all_plot_noselect)
range(Enrich_all_plot_noselect$LogPadjust) #1.000085 54.801126
...
ggplot(Enrich_all_plot_noselect,
  ...
  theme_classic()+labs(x="",y="GO terms",title="Abortion related change DEGs BP enrichment")+
...
write.table(Enrich_all_plot_noselect, file="/mnt/data/chenwei/gongchen/3.seurat_result/figure_two_result/enrichment_result/wait_selected_major_subgroup_all_GO_BP_merge_padjust01.txt",row.names=T, col.names=T) 
